...
def check_email(e_driver, mail_cache):
    ...
    mail_window = e_driver.current_window_handle
    counter = 1
...
#                print('mail_window {}'.format(mail_window))
#                input("dl_window" + e_driver.title)

                #e_driver.find_element_by_xpath('//a[@class="downloadGo"]').click()

                #e_driver.close()

#                e_driver.switch_to_window(e_driver.window_handles[0])
#                input("switch_to_window " + e_driver.title)
#                e_driver.find_element_by_xpath('//a[@id="back_to_inbox_link"]').click()
#        except Exception:
#            e_driver.switch_to_window(e_driver.window_handles[0])
#            input("switch_to_window ex")
#            e_driver.find_element_by_xpath('//a[@id="back_to_inbox_link"]').click()
    ...
    try:
        ...
        try:
            ...
            if dl_element.text not in mail_cache:
                ...
                while(time_counter<20 and can_move_on == False):
...
def download_link(args, driver, link):
        try:
            ...
            try:
                ...
                try:
                    test_element = driver.find_element_by_xpath("//span[@class='buyItemExtra secondaryText']")
                    print ('the album {} is not free'.format(link))
                    return True
                except Exception:
                    pass
...
def command_line_runner():
    ...
    if not args['json_file']:
        ...
        with open(args['user']) as data_file:#'/home/rofler/data.json') as data_file:    
